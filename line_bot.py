...
def build_help_message():
    return (
        ...
        "🌐 AI Assistant\n"
        "════════════════════\n"
        "👉 พิมพ์ชื่อเมนูหลัก (เช่น 'เอกสาร' หรือ 'การเงิน') เพื่อดูรายละเอียดวิธีใช้งานครับ"
    )
...
def build_submenu_message(category: str) -> str:
    ...
    if cat in {"เอกสาร", "สรุปเอกสาร", "document", "📄"}:
        return (
            "📄 เมนู: เอกสาร\n"
            "════════════════════\n"
            "• พิมพ์ 'สรุปเอกสาร' → สรุปจาก รูปถ่าย/ไฟล์ PDF/ไฟล์ Word (มี 5 โหมด)\n"
            "• พิมพ์ 'สรุปใบเสร็จ' → อ่านข้อมูลและสรุปบิล/ใบเสร็จรับเงิน\n\n"
            "👉 พิมพ์ 'เมนู' เพื่อกลับไปหน้าหลัก\n"
            "👉 พิมพ์ 'ยกเลิก' เพื่อหยุดงานปัจจุบัน"
        )
    elif cat in {"สลิปโอนเงิน", "สลิป", "slip", "ระบบสลิป", "🧾"}:
        return (
            "🧾 เมนู: สลิปโอนเงิน\n"
            "════════════════════\n"
            "• ส่งรูปสลิปตรง ๆ → ตรวจสอบและคำนวณยอดเงินอัตโนมัติ\n"
            "• พิมพ์ 'ดูสลิปทั้งหมด' → แสดงประวัติการบันทึกสลิปของครั้งล่าสุด\n"
            "• พิมพ์ 'ยอดรวมสลิป' → รวมยอดเงินของสลิปในครั้งล่าสุด\n"
            "• พิมพ์ 'สลิปที่ [เลข]' → ขอดูสลิปแยกใบ (เช่น 'สลิปที่ 1')\n\n"
            "👉 พิมพ์ 'เมนู' เพื่อกลับไปหน้าหลัก\n"
            "👉 พิมพ์ 'ยกเลิก' เพื่อหยุดงานปัจจุบัน"
        )
    elif cat in {"การเงิน", "บันทึกการเงิน", "finance", "money", "💰"}:
        return (
            "💰 เมนู: การเงิน\n"
            "════════════════════\n"
            "• พิมพ์บันทึกการเงินทั่วไปได้เลย (เช่น 'ค่าน้ำ 270' หรือ 'เงินเดือน 15000')\n"
            "• พิมพ์ 'สรุปเดือนนี้' → สรุปรายงานรายรับ-รายจ่ายของเดือนปัจจุบัน\n"
            "• พิมพ์ 'รายการล่าสุด' → ตรวจสอบประวัติการทำรายการล่าสุด\n\n"
            "👉 พิมพ์ 'เมนู' เพื่อกลับไปหน้าหลัก"
        )
    elif cat in {"นัดหมาย", "บันทึก", "นัดหมาย & บันทึก", "schedule", "note", "📅"}:
        return (
            "📅 เมนู: นัดหมาย & บันทึก\n"
            "════════════════════\n"
            "• พิมพ์ 'นัด [เรื่อง] [ปี-เดือน-วัน] [เวลา]' → ตั้งเตือนนัดหมาย\n"
            "• พิมพ์ 'ดูนัดหมาย' → แสดงรายการนัดหมายทั้งหมด\n"
            "• พิมพ์ 'บันทึก [ข้อความ]' → บันทึกข้อความทั่วไป (memo)\n"
            "• พิมพ์ 'ดูบันทึก' → แสดงรายการบันทึกทั้งหมด\n"
            "• พิมพ์ 'ลบบันทึก' → ลบประวัติบันทึกข้อความ\n\n"
            "👉 พิมพ์ 'เมนู' เพื่อกลับไปหน้าหลัก"
        )
    elif cat in {"โปรไฟล์", "โปรไฟล์ของฉัน", "profile", "ข้อมูลของฉัน", "👤"}:
        return (
            "👤 เมนู: โปรไฟล์ของฉัน\n"
            "════════════════════\n"
            "• พิมพ์ 'ข้อมูลของฉัน' → แสดงข้อมูลโปรไฟล์และประวัติการเงิน\n"
            "• พิมพ์ 'เปลี่ยนชื่อเป็น [ชื่อ]' → แก้ไขชื่อในโปรไฟล์\n"
            "• พิมพ์ 'ล้างข้อมูลของฉัน' → ลบข้อมูลประวัติการเงินและโปรไฟล์ทั้งหมดออกจากระบบ\n\n"
            "👉 พิมพ์ 'เมนู' เพื่อกลับไปหน้าหลัก"
        )
    elif cat in {"รูปภาพ", "จัดการรูปภาพ", "image", "photo", "🖼️"}:
        return (
            "🖼️ เมนู: รูปภาพ\n"
            "════════════════════\n"
    ...
    elif cat in {"เสียง", "เสียงพูด", "voice", "audio", "🎤"}:
        return (
            "🎤 เมนู: เสียง & การถอดความ\n"
            "════════════════════\n"
            "• ส่งไฟล์เสียง (.mp3, .wav, .m4a ฯลฯ) → แปลงเสียงและสรุปเป็นข้อความ\n"
            "• ส่งข้อความเสียง (Voice Message) → ถอดความและสรุปเป็นข้อความแชท\n\n"
            "👉 พิมพ์ 'เมนู' เพื่อกลับไปหน้าหลัก"
        )
    elif cat in {"ai assistant", "ai", "เอไอ", "ผู้ช่วย", "🌐"}:
        return (
            "🌐 เมนู: AI Assistant\n"
            "════════════════════\n"
            "• พิมพ์คุยหรือถามตอบกับ AI ในเรื่องทั่วไปได้ตลอดเวลา\n"
            "• พิมพ์ 'แปลเป็นอังกฤษ: [ข้อความ]' → แปลคำศัพท์/ประโยคเป็นภาษาอังกฤษ\n\n"
            "👉 พิมพ์ 'เมนู' เพื่อกลับไปหน้าหลัก"
        )
    return ""
